...
public class MotorRA173325 extends Motor{
	public MotorRA173325(Baralho deck1, Baralho deck2, ArrayList<Carta> mao1,
	...
	protected void processarJogada(Jogada umaJogada) throws LamaException {
		...
		case ATAQUE:
			...
			//Quem atacou:
			Carta lac = umaJogada.getCartaJogada();
			int ID = lac.getID();
			...
			//O dano:
			int dano = ((CartaLacaio)lac).getAtaque();
			
			//Lacaio invalido
			if(lacaios.contains(lac)){
				//TODO: Acabou de entrar em jogo
				if(true) { 
					//Já atacou?
					if(!lacaiosAtacaramID.contains(ID)) {
						lacaiosAtacaramID.add(ID);
						
						//Processar dano
						if(alvo != null) {//Caso alvo seja lacaio
							
							//Alvo valido?
							if(lacaiosOponente.contains(alvo)) {
								int vidaAlvo = ((CartaLacaio)alvo).getVidaAtual();
								((CartaLacaio)alvo).setVidaAtual(vidaAlvo - dano);
								vidaAlvo -= dano;
								
								imprimir("Lacaio: " +alvo+ "levou dano;\n\tVida Atual: "+vidaAlvo);
								
								//Alvo Morreu
								if(vidaAlvo <= 0) {
									lacaiosOponente.remove(alvo);
									
									imprimir("Lacaio: "+alvo+" morreu");
								}
								
								//Dano recebido
								((CartaLacaio)lac).setVidaAtual(((CartaLacaio)lac).getVidaAtual() - ((CartaLacaio)alvo).getAtaque());
								
								//Carta Morreu
								if(((CartaLacaio)lac).getVidaAtual() <= 0) {
									lacaios.remove(lac);
								}
							} else {
								String erroMessage = new String("carta_id: "+ID+" tentou atacar a carta_id(inválida): "+alvo.getID());
								throw new LamaException(8, umaJogada, erroMessage, jogador == 1 ? 2 : 1);
							}
						} else { //Alvo sera o heroi
							if(jogador == 1) {
								mesa.setVidaHeroi2(mesa.getVidaHeroi2() - dano);
								
								imprimir("Heroi 2 levou dano");
								
								if(mesa.getVidaHeroi2() <= 0) {
									imprimir("Heroi 2 morreu");
								}
							} else {
								mesa.setVidaHeroi1(mesa.getVidaHeroi1() - dano);
								
								imprimir("Heroi 1 levou dano");
								
								if(mesa.getVidaHeroi1() <= 0) {
									imprimir("Heroi 1 morreu");
								}
							}
						}
					} else {
						String erroMessage = new String("carta_id:"+ID+" Realizou mais de um ataque");
						throw new LamaException(7, umaJogada, erroMessage, jogador == 1 ? 2 : 1);					
					}
				} else {
					String erroMessage = new String("carta_id:"+ID+" nao poderia atacar este turno");
					throw new LamaException(6, umaJogada, erroMessage, jogador == 1 ? 2 : 1);
				}
			} else {
				String erroMessage = new String("carta_id:"+ID+" invalida");
				throw new LamaException(5, umaJogada, erroMessage, jogador == 1 ? 2 : 1);		
			}
